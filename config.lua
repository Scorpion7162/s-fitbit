...
Config.notifytimems = 60000 -- This is how often the script will check for the player's stats in milliseconds. 60000ms = 1 minute. 1000ms = 1 second.
Config.notifyhunger = 15 -- This is the hunger level that will trigger the notification. 0-100. So this will trigger when the player's hunger is 15 or lower (15%). will trigger every *notifytimems* check.
Config.notifythirst = 15 -- Similar to hunger, but for thirst. This is the thirst level that will trigger the notification. 0-100. So this will trigger when the player's thirst is 15 or lower (15%).
Config.notifystress = 100 -- Similar to hunger, but for stress. This is the stress level that will trigger the notification. 0-100. So this will trigger when the player's stress is 100 or higher. Stress isnt in the server so this will never trigger.
Config.item = "fitbit" -- This is the item that the player needs to have in their inventory for the notifications to trigger. This is case sensitive.

Config.audio = -- This is the audio that will play when the notification triggers. You can find the audio sets and names in the audio folder in the FiveM directory.
{
...
Config.stressnotificationduration = 7000 -- This is how long the stress notification will stay on the screen for in milliseconds. 7000ms = 7 seconds.
Config.hungernotificationduration = 7000 -- This is how long the hunger notification will stay on the screen for in milliseconds. 7000ms = 7 seconds.
Config.thirstnotificationduration = 7000 -- This is how long the thirst notification will stay on the screen for in milliseconds. 7000ms = 7 seconds
-- DURATIONS END.
